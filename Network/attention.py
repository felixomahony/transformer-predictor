...
class MultiHeadAttention(nn.Module):
    def __init__(self, embed_dim, num_heads, rope=False, grid_shape=None, geometric_base=100):
        """
    ...
    def forward(self, x_q, x_k, x_v, mask=None):
        ...
        if mask is not None:
            attention_scores = attention_scores.masked_fill(mask == 0, torch.finfo(torch.float16).min)

        # cut out any inf values
        attention_scores = attention_scores.masked_fill(attention_scores == float("-inf"), torch.finfo(torch.float16).min)
        attention_scores = attention_scores.masked_fill(attention_scores == float("inf"), torch.finfo(torch.float16).max)
...
def test_multi_head_attention():
    ...
    mha = MultiHeadAttention(embed_dim, num_heads, rope=True, grid_shape=(7,7), geometric_base=100)
